...
function checkbox($data, $checked = []) {
    ...
    foreach ($data as $key => $val) {
        ...
        $checkbox .= '<div class="' . $container_class . '"' . $attr_container . '>
			...
			<label for="' . $val['attr']['id'] . '">' . $val['label'] . '</label>
		</div>';
    }
    ...
    foreach ($data as $key => $val) {
        if ($key == 'edit' || $key == 'pilih') {
            ...
            $html .= '<a href="' . $data[$key]['url'] . '" ' . $attr . '>
						<span class="btn-label-icon"><i class="fa fa-edit pe-1"></i></span> ' . ucfirst($key) . '
					</a>';
        } else if ($key == 'delete') {
